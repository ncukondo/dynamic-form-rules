...
import { stringParser, regParser, eof, map, or, seq, skipFirst, skipSecond, lazy, assert, quoted, sepBy, peak, prettyPrintError } from "../simple-parser";
...
test(`map`, () => {
  expect(map(stringParser(`abc`), (value) => value.toUpperCase())("abc", 0)).toEqual({
    ok: true,
    pos: 3,
    value: `ABC`,
  });
...
test(`skipFirst`, () => {
  expect(skipFirst(stringParser(`abc`), stringParser(`def`))("abcdef", 0)).toEqual({
    ok: true,
    pos: 6,
    value: `def`,
  });
...
test(`skipSecond`, () => {
  expect(skipSecond(stringParser(`abc`), stringParser(`def`))("abcdef", 0)).toEqual({
    ok: true,
    pos: 6,
    value: `abc`,
  });
...
test(`skipSecond fail`, () => {
  expect(skipSecond(stringParser(`abc`), stringParser(`def`))("abc", 0)).toEqual({
    ok: false,
    pos: 3,
    expect: `"def"`,
  });
...
test(`assert`, () => {
  expect(assert(stringParser(`abc`), (value) => value === `abc`, `should be abc`)("abc", 0)).toEqual({
    ok: true,
    pos: 3,
    value: `abc`,
  });
...
test(`assert fail`, () => {
  expect(assert(stringParser(`abc`), (value) => value === `def`, `should be def`)("abc", 0)).toEqual({
    ok: false,
    pos: 0,
...
test(`quoted`, () => {
  expect(quoted(stringParser('"'), stringParser(`abc`), stringParser('"'))(`"abc"`, 0)).toEqual({
    ok: true,
    pos: 5,
    value: `abc`,
  });
